...
class BleDfuServer(QtCore.QObject):
    ...
    def scan_and_connect(self):
        ...
        self.ble_conn = bluepy.btle.Peripheral(self.target_mac, "random")
    ...
    def _dfu_state_set(self, opcode):
        ...
        if len(opcode) == 5:
            ...
            send_opcode = opcode
        self.ble_conn.writeCharacteristic(
            self.ctrlpt_handle, str(chr(int(send_opcode[2:4], 16))) + str(chr(int(send_opcode[4:6], 16))), True
        )
    ...
    def switch_in_dfu_mode(self):
        ...
        if not self._dfu_enable_cccd(False):  # Try this
            return False
        time.sleep(0.5)
    ...
    def _check_DFU_mode(self):
        ...
        if DFUversion_char is not None:
