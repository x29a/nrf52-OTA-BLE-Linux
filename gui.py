...
class Ui_MainWindow(QtCore.QObject):
    ...
    def setupUi(self, MainWindow):
        ...
        self.listWidget.setStyleSheet(
            "QListWidget::item::hover {background: lightgray} QListWidget::item::selected {background: lightblue; border: 1px solid blue}")
    ...
    def selectFile(self):
        ...
        if dlg.exec_():
            ...
            for name in filenames:
                ...
                if name[-4:] == ".dat":
                    self.dat_file = name
            if (self.zip_file != "" and self.bin_file != "") or (self.zip_file != "" and self.hex_file != "") or (
                    self.bin_file != "" and self.hex_file != ""):
    ...
    def start(self):
        ...
        if not self.dfu_in_progress:
            try:
                _thread.start_new_thread(self.dfuProcess, ())
            except:
...
--------------------------------------------------------------------------
    ...
    def _dfu_state_set(self, opcode):
        ...
        if len(opcode) == 5:
            ...
            send_opcode = opcode
        self.ble_conn.writeCharacteristic(self.ctrlpt_handle,
    ...
    def _dfu_state_set_byte(self, opcode):
        ...
        self.ble_conn.writeCharacteristic(self.ctrlpt_handle, str(chr(int(opcode[2:4], 16))), True)
    ...
    def _dfu_pkt_rcpt_notif_req(self):
        ...
        self.ble_conn.writeCharacteristic(self.ctrlpt_handle, str(chr(0x08)) + str(chr(0x0A)) + str(chr(0x00)), True)
    ...
    def _dfu_check_mode(self):
        ...
        if resetHandle or self.ctrlpt_handle:
            if resetHandle:
                ...
                self.ble_conn.writeCharacteristic(resetHandle, str(chr(0x01)), True)
    ...
    def switch_in_dfu_mode(self):
        ...
        self.ble_conn.writeCharacteristic(self.reset_handle, str(chr(0x01)) + str(chr(0x04)), True)
        self.ble_conn.disconnect()
        time.sleep(1)
